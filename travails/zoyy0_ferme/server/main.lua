...
if Config.MaxInService ~= -1 then
	TriggerEvent('esx_service:activateService', 'farm', Config.MaxInService)
end

TriggerEvent('esx_phone:registerNumber', 'farm', _U('farmer_client'), true, true)
TriggerEvent('esx_society:registerSociety', 'farm', 'zoyy0', 'society_farm', 'society_farm', 'society_farm', {type = 'private'})
...
AddEventHandler('esx_farmerjob:startHarvest', function(zone)
	...
	if PlayersHarvesting[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~N\'essayez pas d\'abuser des défauts~w~')
...
AddEventHandler('esx_farmerjob:stopHarvest', function()
	...
	if PlayersHarvesting[_source] == true then
		PlayersHarvesting[_source]=false
		TriggerClientEvent('esx:showNotification', _source, 'Vous avez quitté la zone de récolte')
	else
		TriggerClientEvent('esx:showNotification', _source, 'Vous pouvez récolter du grain')
		PlayersHarvesting[_source]=true
	end
end)
...
AddEventHandler('esx_farmerjob:stopTransform', function()
	...
	if PlayersTransforming[_source] == true then
		PlayersTransforming[_source]=false
		TriggerClientEvent('esx:showNotification', _source, 'Vous avez quitté la zone de traitement')
		
	else
		TriggerClientEvent('esx:showNotification', _source, 'Vous pouvez transformer vos grains')
		PlayersTransforming[_source]=true
...
AddEventHandler('esx_farmerjob:stopSell', function()
	...
	if PlayersSelling[_source] == true then
		PlayersSelling[_source]=false
		TriggerClientEvent('esx:showNotification', _source, 'Vous avez quitté la zone de vente')
		
	else
		TriggerClientEvent('esx:showNotification', _source, 'Vous pouvez vendre vos grains')
		PlayersSelling[_source]=true
	end
...
AddEventHandler('esx_farmerjob:getStockItem', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_farm', function(inventory)
