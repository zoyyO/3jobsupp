...
function SelectOrange()
    local index = GetRandomIntInRange(1,  #Config.Orange)

    for k,v in pairs(Config.Zones) do
      if v.Pos.x == Config.Orange[index].x and v.Pos.y == Config.Orange[index].y and v.Pos.z == Config.Orange[index].z then
        return k
      end
    end
end
...
function StartNPCJob()
    NPCTargetOrange     = SelectOrange()
    local zone            = Config.Zones[NPCTargetOrange]
  
    Blips['NPCTargetOrange'] = AddBlipForCoord(zone.Pos.x,  zone.Pos.y,  zone.Pos.z)
    SetBlipRoute(Blips['NPCTargetOrange'], true)
...
function StopNPCJob(cancel)
    ...
    if Blips['NPCTargetOrange'] ~= nil then
      RemoveBlip(Blips['NPCTargetOrange'])
      Blips['NPCTargetOrange'] = nil
	end
    ...
    if cancel then
	  ESX.ShowNotification(_U('cancel_mission'))
		...
		TriggerServerEvent('esx_fruitier:GiveItem')
		StartNPCJob()
		Done = true
	end   
end
...
AddEventHandler('esx_fruitier:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_fruitier:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_fruitier:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
-- Détection de l'entrer/sortie de la zone du joueur
Citizen.CreateThread(function()
	while true do
		Wait(1)
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_fruitier:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_fruitier:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('esx_fruitier:startVente')
					end
