...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('brasseur_clothes_civil'), value = 'citizen_wear'},
				{label = _U('brasseur_clothes_vine'), value = 'vigne_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'brasseur_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'mule2'},
		}
...
function OpenMobileBrasseurActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_brasseur_actions',
		{
			title    = 'brasseur',
			align    = 'top-left',
			elements = {
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_brasseurjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_brasseurjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_brasseurjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_brasseurjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_brasseurjob:hasEnteredMarker', function(zone)
	if zone == 'OrgeFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'orge_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementBierre' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'bierre_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementLeffe' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'leffe_traitement'
		CurrentActionMsg  = _U('press_traitement_bierre')
		CurrentActionData = {zone = zone}
	end
	...
	if zone == 'BrasseurActions' and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
		CurrentAction     = 'brasseur_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_brasseurjob:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementBierre' or zone == 'TraitementLeffe') and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' then
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.BrasseurActions.Pos.x, Config.Zones.BrasseurActions.Pos.y, Config.Zones.BrasseurActions.Pos.z)
...
-- Entrer / Quitter les évenements de marqueur
Citizen.CreateThread(function()
	while true do
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'brasseur' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'brasseur_actions_menu' then
